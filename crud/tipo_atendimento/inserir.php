...
if(isset($_POST['setor']) && isset($_POST['tipo'])){
	$tipo = $_POST['tipo'];
	$setor = $_POST['setor'];
	if(mysqli_query($conn, "INSERT INTO tipo_atendimento (tipo, setor) values ('$tipo', $setor)")){
		ob_start();
	...
	<?php if($login === 'admin'): ?>		
		<div class="container mt-3">
		  <div class="bg-light rounded pb-3">
			<h1 class="text-center pt-3" id="topo">Inserir tipo de atendimento</h1>
			
			<?php
			$sql = "SELECT max(codigo) as codigo FROM tipo_atendimento";
			$result = mysqli_query($conn, $sql);
			?>	
			<form class="mx-5 mb-3" method="post" action="<?= $_SERVER['PHP_SELF'] ?>">
				<label>Código:</label>
				<input class="form-control" type="number" name="codigo" value="<?= mysqli_fetch_assoc($result)["codigo"] + 1 ?>" readonly>
				...
				<div class="input-group">
					<button class="btn btn-sm btn-primary col-6 mt-2" type="submit">Inserir</button>
