	...
	<?php if($login === 'admin'): ?>		
		<div class="container mt-3">
		  <div class="bg-light rounded pb-3">
			<h1 class="text-center pt-3" id="topo">Lista de endereços</h1>
			<p class="text-muted text-center pb-0 mb-0">
			Para manter a integridade dos dados, não é possível inserir ou excluir endereços.
			</p>
			<p class="text-muted text-center">
			A edição está travada para os endereços sem nome definido na base do Geodados.
			</p>
			
			<?php
			$sql = "SELECT * FROM endereco WHERE logradouro REGEXP '^RUA [0-9]+'";
			  ...
			  <tbody>
				<?php while($row = mysqli_fetch_assoc($result)): ?>
					<tr>
						...
						<td><?= $row["numero_residencia"] ?></td>
						<td><?= $row["bairro"] ?></td>
						<td class="d-flex justify-content-center">
						<a href="endereco/editar.php?i=<?= $row['codigo'] ?>" class="col-4 btn btn-sm btn-primary">Editar</a>
						<a href="../geojson/view.php?lg=<?= $row['logradouro'] . ' '?>&n=<?= $row["numero_residencia"] ?>" class="col-4 btn btn-sm btn-secondary">Ver no mapa</a>
						</td>
					</tr>
				<?php endWhile; ?>
			  </tbody>
